...
export class ProductsComponent implements OnInit {
  ...
  onFilter(productName: string, categoryId: string){
    ...
    if(productName === null && categoryId === null){
      ...
      this.productsService.getFilteredProducts(productName, categoryId, this.page, null, null).subscribe(response => {
        this.products = response;
        this.productsService.getFilteredProducts(productName, categoryId, 1, null, null).subscribe(response => {
  ...
  onClickNameSort(){
    ...
    if(this.nameSortClicks === 2){
      ...
      this.productsService.getFilteredProducts(productName, categoryId, this.page, sortBy, sortDirection).subscribe(response =>{
        this.products = response;
        this.filtered = true;
        console.log(this.products);
      });
  ...
  onPageChanged(page: number){
    ...
    this.productsService.getFilteredProducts(productName, categoryId, page, sortBy, sortDirection).subscribe(response =>{
      this.products = response;
      this.filtered = false;
    });
    this.page = page;
